...
var fragmentShader = `precision lowp float;uniform float y;float x,z;vec3 f;vec2 r,c=vec2(640,360);vec3 s(vec3 v,vec3 y){vec3 r=sin(y),f=cos(y);v=vec3(f.z*v.x-r.z*v.y,r.z*v.x+f.z*v.y,v.z);v=vec3(f.y*v.x+r.y*v.z,v.y,-r.y*v.x+f.y*v.z);return vec3(v.x,f.x*v.y-r.x*v.z,r.x*v.y+f.x*v.z);}float s(vec3 v){return v+=sin(s(v,vec3(y))*2.)*.08,v=s(v,vec3(-y)),z=length(tan(v*20.)/4.),abs(dot(sin(v),cos(v.zxy))-2.8)-1.8;}void main(){r=gl_FragCoord.xy/c.xy*2.-1.;r.x*=c.x/c.y;for(int v=0;v<=64;v++){f=vec3(y/6.2)+normalize(vec3(0.,-.5145,-.8575)+r.x*vec3(-.75,0.,0.)+r.y*vec3(0.,.6443,-.3859))*x;float l=s(f);x+=l;vec2 n=vec2(.1,0.);f=normalize(vec3(s(f+n.xyy)-l,s(f+n.yxy)-l,s(f+n.yyx)-l));}gl_FragColor=vec4(z*f*max(dot(f,vec3(0.,1.,1.)),0.)*vec3(smoothstep(25.,0.,x)),1.);}`;
